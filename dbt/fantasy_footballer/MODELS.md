...
| `int__owner_career_summary` | owner (`owner_id` unique) | Career roll-ups per owner: tennis-style title counts, cumulative lucky/unlucky/shotgun + clutch W-L totals, `seasons_played` (tenure), `snub_total`/`luck_in_total` (career snubbed/lucked-in seasons), `points_total`/`games_total`/`points_per_game`/`clutch_win_pct`, and `acquisitions_total`/`trades_total` (career roster moves, summed from `base_s001__teams` via `int__owner_team_year_map`). Keeps `all_time_records` a thin select. | `int__season_titles_long`, `int__owner_season_scoring`, `int__season_titles`, `base_s001__teams`, `int__owner_team_year_map` |
