...
| `int__owner_head_to_head` | owner pair (`owner_opponent_id` unique) | All-time rivalry between two owners: split record (reg vs playoff), all-meeting totals/averages (points for/against, avg margin), single-game extremes as **owner-perspective** score + margin/combined + year/week context (biggest win, closest game, highest shootout, lowest slugfest — most carrying a winner `_leader` id — via `arg_max`/`arg_min`), streaks (current + longest win via gaps-and-islands), owner-perspective `last_meeting_context`, clutch-meeting record, `held_under_100`, and per-opponent `lucky_wins`/`unlucky_losses` (joined from `int__lucky_records` on `team_week_id`). Summed across opponents the reg record yields each owner's career W-L-T. Backs the H2H dashboard. | `int__team_week_results`, `int__lucky_records` |
