...
sections that both stats pages use),
...
|---|---|---|---|---|
| `/` | `splash/home.py` | public | — | **The hub.** A section-tile grid (`section_tile`) — the site's front door to every destination, driven by the `SECTION_TILES` list (gated by `get_access_level()`). Stats destinations link straight to their `/stats_center/*` routes; there is no `/stats_center` landing. |
...
| `/owner_history/{owner_id}/{year}` | `owner_history/spotlight.py` | 0 | `season_overview`, `season_schedule`, `season_highlights`, `owner_weekly_lineup`, `postseason_schedule` | `season_overview_card`s; queries `where owner_id=.. and year=..` then `[0]`. **Highlights card** (`highlights_card`) lists that owner-year's podium finishes from `season_highlights` (medal by `rank`, value tinted by `SECTION_COLORS`, subtitle = the mart's precomposed `detail`); empty-state when none. Below the overview sits a **tabbed section** (`ui.tabs`, consistent `min-height`) with three tabs: **Regular Season** (`season_schedule_table`) — the dense `table()` q-table with a `body-cell-Highlights` slot rendering one section-colored chip (icon + tooltip) per active week flag, driven by `SCHEDULE_FLAGS` (flag → icon/section/label), the boolean flag columns from `season_schedule` passed as `hidden_fields` and read by the slot; **each row is clickable** (`on_week_click`) and jumps the Roster tab to that week. **Roster** (`weekly_roster_view`) — a week `ui.select` (+ **All**, postseason weeks labeled `· PO`) driving an actual-vs-optimal lineup view from `owner_weekly_lineup`: two columns (`_lineup_column`) by `LINEUP_SLOT_ORDER` with the started-but-suboptimal players marked red (left accent border + bright accent text, readable on the dark page) and the should-have-started bench players marked green, plus that week's **points left on the bench** (postseason weeks tagged `· Postseason`); **All** renders the season roster (`_season_roster` from `owner_roster_production`: player names + Weeks/Started/Rostered Pts/Started Pts/Util%). (The dedicated per-player page is `/stats_center/roster_production`.) **Postseason** (`postseason_schedule_table`) — that owner-season's bracket games (`postseason_schedule`: round label, opponent team + owner, both scores); **each row is clickable** and jumps the Roster tab to that postseason week. Empty state when the owner missed the bracket. |
